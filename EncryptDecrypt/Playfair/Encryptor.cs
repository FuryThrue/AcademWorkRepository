    ...
    class Encryptor
    {
        public string Encrypt(string message, string[,] key)
        {
            ...
            for (int i = 0; i < message.Length; i += 2)
            {
                ...
                var encryptedPart = EncryptCouple(piece, key);
                builder.Append(encryptedPart);
            }
            return builder.ToString();
        }

        private string EncryptCouple(string twoCharString, string[,] key)
        {
            ...
            {
                var firstIndexofColumn = dataFirst.Item2 + 1 > key.GetLength(0) - 1 ? 0 : dataFirst.Item2 + 1;
                firstChar = key[dataSecond.Item1, firstIndexofColumn];

                var secondIndexofColumn = dataSecond.Item2 + 1 > key.GetLength(1) - 1 ? 0 : dataSecond.Item2 + 1;
                secondChar = key[dataSecond.Item1, secondIndexofColumn];
            }
                else if (dataFirst.Item2 == dataSecond.Item2)
                {
                    var firstIndexofRow = dataFirst.Item1 + 1 > key.GetLength(0) - 1 ? 0 : dataFirst.Item1 + 1;
                    firstChar = key[firstIndexofRow, dataSecond.Item2];

                    var secondIndexofRow = dataSecond.Item1 + 1 > key.GetLength(1) - 1 ? 0 : dataSecond.Item1 + 1;
